...
# SPECIFICATION: train a deep neural net to recognize images of clothing.
# FOR: CS 4210- Assignment #4
# TIME SPENT: this file took 40 mins.
#-----------------------------------------------------------*/
...
#Iterate here over number of hidden layers, number of neurons in each hidden layer and the learning rate to find highestAccuracy.
#-->add your Pyhton code here
...
highestAccuracy = 0
bestModel = build_model(1, 1, 1, 1)

for hiddenInst in n_hidden :                          #looking or the best parameters w.r.t the number of hidden layers
    for neuronsInst in n_neurons :                      #looking or the best parameters w.r.t the number of neurons
        for rateInst in l_rate :                  #looking or the best parameters w.r.t the learning rate

            #build the model for each combination by calling the function:
            #model = build_model()
            #-->add your Pyhton code here
            
            model = build_model(0, hiddenInst, neuronsInst, rateInst)
            ...
            #     print(f'{i}th item: {evaluation[i]}\n')
            
            currentAccuracy = evaluation[1]

            if currentAccuracy > highestAccuracy:
                highestAccuracy = currentAccuracy
                bestModel = evaluation[1]
                
                print("Highest accuracy so far: " + str(highestAccuracy))
                print("Parameters: " + "Number of Hidden Layers: " + str(hiddenInst) + ",number of neurons: " + str(neuronsInst) + ",learning rate: " + str(rateInst))
                print()
